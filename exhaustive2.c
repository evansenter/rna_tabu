...
double chk_if_min(){
  ...
  //Check for too large internal loop (to be consistent w/ our progs)
  for (k=1; k<num_bp; k++){
    i=pair_list[k][0];j=pair_list[k][1];
    ip=pair_list[k+1][0];jp=pair_list[k+1][1];
    if ( jp>j || (ip-i+j-jp)<=30) continue;
    //So, we have a big gap.  Rest is to make sure an IL, not a ML.
    for (l=k+2;pair_list[l][1]<jp && l<=num_bp; l++)
      ;
    if (l>num_bp || pair_list[l][1]>j) return 0;
    //at this point, all conditions for a too large internal loop
    // are satisfied.
  }
